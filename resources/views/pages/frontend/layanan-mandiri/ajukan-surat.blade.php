    ...
    <div x-data="{
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                ...
                <p class="text-sm text-gray-500">Ajukan berbagai keperluan surat pengantar/keterangan desa secara mandiri.</p>
            </div>
            ...
            <div class="flex items-center gap-3 sm:gap-4 shrink-0">
                <div class="flex flex-col items-center gap-1">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-colors"
                         :class="step >= 1 ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-gray-400'">1</div>
                    <span class="text-[10px] font-semibold" :class="step >= 1 ? 'text-green-700' : 'text-gray-400'">Pilih Surat</span>
                </div>
                <div class="w-8 sm:w-12 h-0.5 transition-colors mb-4" :class="step >= 2 ? 'bg-green-500' : 'bg-gray-200'"></div>
                <div class="flex flex-col items-center gap-1">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-colors"
                         :class="step >= 2 ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-gray-400'">2</div>
                    <span class="text-[10px] font-semibold" :class="step >= 2 ? 'text-green-700' : 'text-gray-400'">Isi Formulir</span>
                </div>
                <div class="w-8 sm:w-12 h-0.5 transition-colors mb-4" :class="step >= 3 ? 'bg-green-500' : 'bg-gray-200'"></div>
                <div class="flex flex-col items-center gap-1">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-colors"
                         :class="step >= 3 ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-gray-400'">
                        <i class="fa-solid fa-check"></i>
                    </div>
                    <span class="text-[10px] font-semibold" :class="step >= 3 ? 'text-green-700' : 'text-gray-400'">Selesai</span>
                </div>
            </div>
        </div>
        ...
        @if(session('success'))
            <div x-init="step = 3" x-show="step === 3" class="py-12">
                <div class="max-w-md mx-auto bg-white rounded-3xl p-8 border border-gray-100 shadow-lg text-center relative overflow-hidden">
                    <div class="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-green-50 to-white -z-0"></div>
                    <div class="relative z-10">
                        <div class="w-20 h-20 bg-green-100 text-green-500 rounded-full flex items-center justify-center text-4xl mx-auto mb-6 border-4 border-white shadow-sm">
                            <i class="fa-solid fa-check"></i>
                        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                <template x-for="surat in filteredSuratList" :key="surat.id">
                    <div @click="selectedSurat = surat.id"
                         ...
                         :class="selectedSurat === surat.id ? 'border-green-500 bg-green-50/30' : 'border-gray-100 hover:border-green-200'">
                        <div class="w-12 h-12 rounded-xl mb-4 flex items-center justify-center text-xl transition-colors"
                             :class="selectedSurat === surat.id ? 'bg-green-500 text-white' : 'bg-gray-50 text-gray-500 group-hover:bg-green-100 group-hover:text-green-600'">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center">
                            ...
                            <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs transition-colors"
                                  :class="selectedSurat === surat.id ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-green-100 group-hover:text-green-600'">
        ...
        ═══════════════════════════════════════════════════════ --}}
        <div x-show="step === 2 && !{{ session('success') ? 'true' : 'false' }}" x-transition.opacity.duration.300ms x-cloak>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <header class="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                            ...
                            <span class="text-[10px] bg-blue-100 text-blue-700 font-bold px-2 py-0.5 rounded uppercase">Auto-filled</span>
                        </header>
                        <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" value="{{ $warga->tempat_lahir }}, {{ $warga->tanggal_lahir?->format('d-m-Y') ?? '-' }}" readonly
                                       class="w-full bg-gray-50 border border-transparent rounded-lg px-3 py-2 text-sm text-gray-700 outline-none cursor-not-allowed">
                            </div>
                    ...
                    {{-- Detail Surat --}}
                    <section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden border-t-4 border-t-green-500">
                        <header class="px-5 py-4 border-b border-gray-100 bg-white">
                            <h3 class="font-bold text-gray-800 text-sm uppercase tracking-wide">Detail <span x-text="selectedSuratObj?.nama ?? ''"></span></h3>
                        ...
                        <div class="p-5 space-y-5">
                            ...
                            <div x-show="selectedSurat === 'pengantar_usaha'" x-collapse x-cloak>
                                <label for="nama_usaha" class="block text-xs font-bold text-gray-700 mb-1">Nama Usaha/Toko <span class="text-red-500">*</span></label>
                            ...
                            <div x-show="['domisili', 'sktm'].includes(selectedSurat)" x-collapse x-cloak>
                                <label for="berlaku_hingga" class="block text-xs font-bold text-gray-700 mb-1">Berlaku Hingga (Opsional)</label>
                            ...
                            <div class="border-t border-gray-100 pt-5 mt-2 space-y-4">
                                <div>
                                    <label for="keperluan" class="block text-xs font-bold text-gray-700 mb-1">Tujuan / Keperluan Surat <span class="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label for="catatan" class="block text-xs font-bold text-gray-700 mb-1">Catatan Tambahan (Opsional)</label>
                                ...
                                <div class="flex gap-3">
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" name="prioritas" value="normal"
                                               ...
                                               class="sr-only peer">
                                        <div class="flex items-center gap-2 border-2 border-gray-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 rounded-xl p-3 transition-all">
                                    ...
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" name="prioritas" value="tinggi"
                                               ...
                                               class="sr-only peer">
                                        <div class="flex items-center gap-2 border-2 border-gray-200 peer-checked:border-red-500 peer-checked:bg-red-50 rounded-xl p-3 transition-all">
                    ...
                    <section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden sticky top-6">
                        ...
                        <div class="p-5 space-y-4">
                            <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                                ...
                                <span class="text-xs font-bold text-gray-900 text-right w-1/2 leading-tight" x-text="selectedSuratObj?.nama ?? '-'"></span>
                            </div>
                            <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                            ...
                            <label class="flex items-start gap-3 mt-4 cursor-pointer group">
                                <div class="relative flex items-center shrink-0 mt-0.5">
                                    ...
                                    <i class="fa-solid fa-check absolute text-white text-[10px] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 peer-checked:opacity-100 pointer-events-none"></i>
                                </div>
                                <span class="text-[10px] leading-relaxed text-gray-600 group-hover:text-gray-900 select-none">
                                    Saya menyatakan bahwa data dan dokumen yang saya kirimkan adalah benar dan sah sesuai hukum.
                                </span>
                            </label>
