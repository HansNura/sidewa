...
@section('title', 'Data Agama - Desa Sindangmukti')
...
<script>
    function dataAgama() {
        return {
            statistikAgama: @json($statistikAgama),

            getTotalPenduduk() {
                const totalStat = this.statistikAgama.find(
                    (s) => s.isHighlight
                );
            ...
            initAgamaChart() {
                ...
                const chartData = this.statistikAgama.filter(
                ...
                const highchartsData = chartData.map((item) => {
                    return {
                        name: item.label,
                        y: item.value,
                        sliced: item.value === totalPenduduk,
                        selected: item.value === totalPenduduk,
                    };
                });

                Highcharts.chart("agama-pie-chart", {
                    chart: {
                        type: "pie",
                    },
                    title: {
                        text: "Proporsi Agama Penduduk",
                        align: "center",
                    },
                    subtitle: {
                        text:
                            "Data Per 3 November 2025. Total Penduduk: " +
...
@section('content')
<main class="bg-gray-50 pt-16" x-data="dataAgama()" x-init="setHighchartsTheme(); initAgamaChart();">
    ...
    <section class="py-16">
        <div class="px-6 mx-auto max-w-7xl lg:px-8">
            <div class="w-full p-6 mx-auto bg-white shadow-lg rounded-xl md:p-8">
                <div id="agama-pie-chart" style="height: 500px"></div>
            </div>
        </div>
    </section>
    ...
    <section class="pb-16 -mt-8" x-data="{ total: getTotalPenduduk() }">
        <div class="px-6 mx-auto space-y-6 max-w-7xl lg:px-8">
            <h2 class="mb-6 text-2xl font-bold text-center text-[#1f2937]">
                Rincian Statistik Agama
            </h2>

            @php
                $highlightStat = collect($statistikAgama)->firstWhere('isHighlight', true);
                $otherStats = collect($statistikAgama)->where('isHighlight', false);
            @endphp
