...
@section('content')
<main class="bg-gray-50 pt-16" x-data="dataWilayah()" x-init="setHighchartsTheme(); initMap(); initWilayahColumnChart();">
                        ...
                        @foreach($statistikWilayah as $stat)
                            <div class="flex items-center p-4 space-x-4 transition-all bg-white shadow-lg rounded-xl hover:shadow-xl text-[#1f2937]">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="text-2xl font-bold text-gray-900">{{ is_numeric($stat['value']) ? number_format($stat['value'], 0, ',', '.') : $stat['value'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
