...
<section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    {{-- Total Balita --}}
    <article
        class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col justify-center relative overflow-hidden group">
        <div
    ...
    {{-- Stunting Cases --}}
    <article
        class="bg-white rounded-2xl p-5 border border-red-100 shadow-sm flex flex-col justify-center relative overflow-hidden group">
        <div
        ...
        @if ($sangatPendek > 0)
            <p class="text-xs font-medium text-red-500 mt-2 relative z-10">{{ $sangatPendek }} Kasus Kategori Sangat
                Pendek</p>
        @endif
    </article>
    ...
    {{-- Trend --}}
    <article
        class="bg-white rounded-2xl p-5 border {{ $trend <= 0 ? 'border-green-100' : 'border-red-100' }} shadow-sm flex flex-col justify-center relative overflow-hidden group">
        <div
            class="absolute -right-4 -top-4 w-16 h-16 {{ $trend <= 0 ? 'bg-green-50' : 'bg-red-50' }} rounded-full group-hover:scale-150 transition-transform duration-500">
        </div>
        <div class="relative z-10 flex justify-between items-start">
            <div>
            ...
            <div
                class="w-10 h-10 rounded-full {{ $trend <= 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }} flex items-center justify-center shrink-0">
