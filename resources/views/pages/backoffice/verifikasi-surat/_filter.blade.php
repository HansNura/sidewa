    ...
    <form method="GET" action="{{ route('admin.verifikasi-surat.index') }}" class="flex flex-col md:flex-row gap-4">
        ...
        <div class="w-full md:w-56 relative shrink-0">
            <select name="status" onchange="this.form.submit()"
                ...
                <option value="menunggu_tte" {{ ($selStatus ?? '') === 'menunggu_tte' ? 'selected' : '' }}>Menunggu TTE ({{ $countTTE }})</option>
                <option value="verifikasi" {{ ($selStatus ?? '') === 'verifikasi' ? 'selected' : '' }}>Menunggu Verifikasi ({{ $countVerifikasi }})</option>
            </select>
