...
<div x-show="activeTab === 'list'" x-transition.opacity class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <form action="{{ route('admin.informasi.index') }}" method="GET" class="flex-1 relative flex gap-4 w-full flex-col md:flex-row">
            <input type="hidden" name="tab" value="list">
            
            <div class="flex-1 relative">
                <i class="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul kegiatan atau pengumuman..." class="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-all">
            </div>
            <select name="type" onchange="this.form.submit()" class="w-full md:w-48 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none cursor-pointer font-medium text-gray-700">
    ...
    <div class="bg-white border border-gray-100 shadow-sm rounded-2xl overflow-hidden">
        ...
        <div x-show="selectedRows.length > 0" x-collapse x-cloak class="bg-green-50 border-b border-green-100 px-5 py-3 flex items-center justify-between">
            ...
            <form action="{{ route('admin.informasi.bulk') }}" method="POST" id="bulkActionForm" class="flex gap-2">
                ...
                <button type="submit" name="action" value="archive" class="text-xs font-bold px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Arsipkan</button>
                <button type="submit" name="action" value="delete" onclick="return confirm('Hapus permanen item terpilih?')" class="text-xs font-bold px-3 py-1.5 bg-red-600 border border-red-700 text-white rounded-lg hover:bg-red-700">Hapus</button>
            </form>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse whitespace-nowrap">
                ...
                <tbody class="text-sm divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $item->status == \App\Models\InformasiPublik::STATUS_ARCHIVED ? 'opacity-80' : '' }}">
                        ...
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                @if($item->type == \App\Models\InformasiPublik::TYPE_AGENDA)
                                    <div class="w-10 h-10 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center border border-purple-100 shadow-sm shrink-0">
                                        <i class="fa-regular fa-calendar-check text-xl"></i>
                                    </div>
                                    <div>
                                        <div class="font-bold text-gray-900 leading-tight w-48 sm:w-64 md:w-80 truncate" title="{{ $item->title }}">{{ $item->title }}</div>
                                        <span class="text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-bold uppercase mt-1 inline-block">Agenda</span>
                                    </div>
                                @else
                                    <div class="w-10 h-10 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-100 shadow-sm shrink-0">
                                        <i class="fa-solid fa-bullhorn text-xl"></i>
                                    </div>
                                    <div>
                                        <div class="font-bold text-gray-900 leading-tight w-48 sm:w-64 md:w-80 truncate" title="{{ $item->title }}">{{ $item->title }}</div>
                                        <span class="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-bold uppercase mt-1 inline-block">Pengumuman</span>
                                    </div>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="p-4">
                            @if($item->type == \App\Models\InformasiPublik::TYPE_AGENDA)
                                <span class="text-xs font-semibold text-gray-700">{{ $item->location ?? '-' }}</span>
                            @else
                                <span class="text-xs text-gray-400 italic">- (Publik)</span>
                            @endif
                        </td>
                        <td class="p-4 text-center">
                            @if($item->status == \App\Models\InformasiPublik::STATUS_PUBLISH)
                                <span class="bg-green-50 text-green-700 text-[9px] font-black px-2.5 py-1 rounded-full border border-green-200 uppercase tracking-widest block mb-1">Published</span>
                            @elseif($item->status == \App\Models\InformasiPublik::STATUS_DRAFT)
                                <span class="bg-gray-100 text-gray-500 text-[9px] font-black px-2.5 py-1 rounded-full border border-gray-200 uppercase tracking-widest block mb-1">Draft</span>
                            @else
                                <span class="bg-gray-100 text-gray-600 text-[9px] font-black px-2.5 py-1 rounded-full border border-gray-200 uppercase tracking-widest block mb-1">Archived</span>
                            @endif
                    ...
                    <tr>
                        <td colspan="6" class="p-12 text-center text-gray-400">
                            <i class="fa-solid fa-file-invoice text-3xl mb-3 opacity-30"></i>
