        ...
        <form :action="modalMode === 'edit'
            ...
            <div class="overflow-y-auto custom-scrollbar flex-1 flex flex-col md:flex-row">
                ...
                <div class="w-full md:w-1/3 p-6 border-b md:border-b-0 md:border-r border-gray-100 bg-white">
                    <h4 class="text-sm font-bold text-gray-800 mb-4 pb-2 border-b border-gray-100">Informasi Dasar Role</h4>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-3 mt-4">
                            ...
                            <p class="text-[10px] text-amber-700 leading-relaxed">
                                Berikan akses secara bijak. Akses 'Delete' disarankan hanya untuk tingkat Manajerial/Admin.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-full md:w-2/3 bg-gray-50/50 flex flex-col">
                    ...
                    <div class="overflow-x-auto p-4 custom-scrollbar flex-1">
                        <table class="w-full text-left border-collapse bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                            <thead>
                            ...
                            <tbody class="text-sm divide-y divide-gray-100">
                                @foreach ($modules as $module)
                                    <tr class="hover:bg-gray-50 transition-colors {{ $module->is_sensitive ? 'bg-red-50/10' : '' }}">
                                        ...
                                        @foreach (['can_view', 'can_create', 'can_edit', 'can_delete'] as $ability)
                                            <td class="p-3 text-center">
                                                <input type="hidden" name="permissions[{{ $module->id }}][{{ $ability }}]" value="0">
                                                <input type="checkbox"
                                                       class="matrix-checkbox mx-auto"
                                                       name="permissions[{{ $module->id }}][{{ $ability }}]"
                                                       value="1"
                                                       x-model="permissionData[{{ $module->id }}] && permissionData[{{ $module->id }}]['{{ $ability }}']"
                                                       :checked="permissionData[{{ $module->id }}] && permissionData[{{ $module->id }}]['{{ $ability }}']"
                                                       @if ($module->is_sensitive && $ability !== 'can_view')
                                                           {{-- Sensitive modules: only view is freely assignable, others need deliberate choice --}}
                                                       @endif
                                                >
                                            </td>
                                        @endforeach
