...
<div x-show="addMemberModalOpen" class="fixed inset-0 z-[130] flex items-center justify-center p-4 !m-0" x-cloak>
    <div x-show="addMemberModalOpen" x-transition.opacity class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm"
        @click="addMemberModalOpen = false"></div>

    <div x-show="addMemberModalOpen" x-transition
        class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto custom-scrollbar flex-1">
            <form :action="`{{ url('admin/kartu-keluarga') }}/${detail?.id}/add-member`" method="POST"
                class="space-y-4" id="addMemberForm">
                @csrf
                ...
                <div class="space-y-1">
                    ...
                    <select name="status_hubungan"
                        class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none cursor-pointer"
                        required>
                        <template x-if="detail?.kepala_jk === 'P' || detail?.kepala_jk === 'Perempuan'">
                            <option value="Suami">Suami</option>
                        </template>
                        <template x-if="detail?.kepala_jk === 'L' || detail?.kepala_jk === 'Laki-laki' || !detail?.kepala_jk">
                            <option value="Istri">Istri</option>
                        </template>
                        @foreach (['Anak', 'Menantu', 'Cucu', 'Orang Tua', 'Mertua', 'Famili Lain'] as $rel)
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-2 shrink-0">
