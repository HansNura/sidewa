<section x-show="activeTab === 'kelola'" x-transition.opacity class="grid grid-cols-1 lg:grid-cols-3 gap-6" x-cloak>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="sumber_dana"
                            ...
                            <option value="BanProv">Bantuan Provinsi</option>
                            <option value="Lainnya">Lainnya</option>
                        </select>
                    </div>
                </div>
                <!-- Checkbox Publikasi -->
                <label class="flex items-center gap-3 bg-green-50/50 border border-green-100 p-3 rounded-xl cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" checked class="w-4 h-4 text-green-600 rounded">
                    <div>
                        <p class="text-sm font-bold text-green-800">Tampilkan di Publik</p>
                        <p class="text-[10px] text-green-700/80">Anggaran akan muncul pada diagram transparansi portal desa.</p>
                    </div>
                </label>

                <div class="pt-4 flex justify-end gap-3">
                    <button type="submit" class="px-8 py-2.5 rounded-xl text-sm font-bold text-white bg-green-700 hover:bg-green-800 shadow-md">
                        Simpan Anggaran
                    </button>
                </div>
            </form>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <h4 class="font-bold text-gray-800 mb-4 flex items-center gap-2"><i class="fa-solid fa-shield-check text-green-600"></i> Aturan Validasi</h4>
            <ul class="space-y-3">
                ...
                <li class="flex items-start gap-3">
                    <i class="fa-solid fa-circle-check text-green-500 mt-1 text-xs"></i>
                    <p class="text-xs text-gray-600">Pendapatan dan Belanja idealnya _balance_ selaras pada tahun berjalan.</p>
                </li>
            </ul>
        </div>
        ...
        <div class="bg-gray-900 rounded-2xl p-5 text-white shadow-xl">
            <h4 class="text-xs font-bold text-green-400 uppercase tracking-widest mb-4">Ringkasan TA {{ $tahun }}</h4>
            <div class="space-y-3">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-400">Total Psn.</span>
                    <span class="font-bold" x-text="formatIDR({{ $summary['pendapatan'] }})">Rp {{ number_format($summary['pendapatan']) }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-400">Total Bln.</span>
                    <span class="font-bold" x-text="formatIDR({{ $summary['belanja'] }})">Rp {{ number_format($summary['belanja']) }}</span>
                </div>
