...
@section('content')
    <div class="flex-1 flex flex-col h-full bg-[#F8FAFC]">
        ...
        <div class="space-y-6" x-data="{
            ...
            openEditModal(item) {
                this.editData = { ...item };
                // ensure boolean check for checkbox mapping if needed
                this.editData.is_published = !!item.is_published;
                this.editBudgetModalOpen = true;
            }
        }">
            ...
            <section
                ...
                <div class="flex flex-wrap gap-2 p-1 bg-gray-100 rounded-xl">
                    <button @click="activeTab = 'overview'"
                        :class="activeTab === 'overview' ? 'bg-white shadow-sm text-green-700 font-bold' :
                    ...
                    <button @click="activeTab = 'kelola'"
                        :class="activeTab === 'kelola' ? 'bg-white shadow-sm text-green-700 font-bold' :
                            'text-gray-500 hover:text-gray-700'"
                        class="px-6 py-2 text-sm rounded-lg transition-all whitespace-nowrap">Input Anggaran</button>

                    <button @click="activeTab = 'media'"
                        :class="activeTab === 'media' ? 'bg-white shadow-sm text-green-700 font-bold' :
                            'text-gray-500 hover:text-gray-700'"
            ...
            <!-- MODAL EDIT -->
            @include('pages.backoffice.keuangan.apbdes._edit-modal')
