<footer class="bg-slate-900 text-slate-400 pt-20 pb-10 border-t border-slate-800" x-data="footerData()">
    <div class="px-6 mx-auto max-w-7xl md:px-10">
        <div class="grid grid-cols-1 gap-12 pb-12 border-b border-slate-800 md:grid-cols-2 lg:grid-cols-4">
            <!-- Identitas Desa -->
            <div class="lg:col-span-2">
                <div class="flex items-center gap-4 mb-6">
                    <div class="p-1.5 bg-white rounded-2xl shadow-sm">
                        <img :src="identity.logoSrc" alt="Logo Desa" class="w-12 h-12 object-contain" />
                    </div>
                    <div>
                        <h2 class="text-2xl font-black text-white tracking-tight" x-text="identity.name"></h2>
                        <p class="text-sm font-medium text-emerald-400 uppercase tracking-widest" x-text="identity.tagline"></p>
                    </div>
                </div>
                <p class="text-sm leading-relaxed max-w-md" x-text="identity.description"></p>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm">
                    <template x-for="link in quickLinks" :key="link.href">
                        <li>
                            <a :href="link.href"
                                class="flex items-center gap-2 group transition-colors hover:text-emerald-400">
                ...
                <ul class="space-y-4 text-sm">
                    <template x-for="item in contactInfo" :key="item.text">
                        <li class="flex items-start gap-3">
                            <span class="w-5 h-5 flex-shrink-0 text-emerald-500" x-html="item.icon"></span>
                ...
                <div class="flex gap-3 mt-8">
                    <template x-for="social in socialLinks" :key="social.title">
                        <a :href="social.href" :title="social.title" target="_blank" rel="noopener noreferrer"
                            class="p-2.5 transition-all duration-300 rounded-xl bg-slate-800 text-slate-400 hover:bg-emerald-500 hover:text-white hover:-translate-y-1 shadow-sm">
        ...
        <!-- Copyright -->
        <div class="flex flex-col md:flex-row items-center justify-between pt-8 text-sm text-slate-500" x-cloak>
            <p>
                &copy; <span x-text="copyright.year"></span> <span x-text="copyright.text"></span>
            </p>
            <p class="mt-4 md:mt-0">
                Dikembangkan oleh <a :href="copyright.developerLink" target="_blank" rel="noopener noreferrer"
                    class="font-medium text-slate-300 transition-colors hover:text-emerald-400"
...
document.addEventListener('alpine:init', () => {
    Alpine.data('footerData', () => ({
        identity: {
            ...
            description: 'Website resmi Pemerintah Desa Sindangmukti sebagai wujud transparansi dan layanan publik berbasis digital untuk mewujudkan tata kelola desa yang lebih baik.'
        },
        quickLinks: [
        ...
        contactInfo: [
            { icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>', text: 'Balai Desa Sindangmukti', href: '#' },
        ...
        copyright: {
            ...
            text: ' Pemerintah Desa Sindangmukti.',
