...
@php
    ...
    $menuGroups = [
        ...
        [
            ...
            'items' => [
                ['label' => 'Monitoring Presensi', 'route' => '#'],
                ['label' => 'Buku Tamu Digital', 'route' => '#'],
            ],
        ],
        [
            'label' => 'Keuangan Desa',
...
<aside class="bg-white border-r border-gray-200 flex flex-col transition-all duration-300 z-30 h-full fixed lg:relative shadow-sm lg:shadow-none"
    ...
    <nav class="flex-1 overflow-y-auto custom-scrollbar py-4 px-3 space-y-1">
        @foreach ($menuGroups as $index => $group)
            @php
                ...
                $isGroupActive = collect($group['items'])->contains(fn ($item) =>
